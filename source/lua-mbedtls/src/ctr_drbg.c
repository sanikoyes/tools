...
static l_mbedtls_ctr_drbg_seed(lua_State *L) {
	...
	size_t len;
	const unsigned char *custom = luaL_checklstring(L, 3, &len);

	lua_pushinteger(L, mbedtls_ctr_drbg_seed(ctx, mbedtls_entropy_func, entropy, custom, len));
	return 1;
}

static l_mbedtls_ctr_drbg_set_prediction_resistance(lua_State *L) {
...
static l_mbedtls_ctr_drbg_set_entropy_len(lua_State *L) {
...
static l_mbedtls_ctr_drbg_set_reseed_interval(lua_State *L) {
...
static l_mbedtls_ctr_drbg_reseed(lua_State *L) {

	mbedtls_ctr_drbg_context *ctx = (mbedtls_ctr_drbg_context *) luaL_checkudata(L, 1, CLASS_NAME);
	size_t len;
	const unsigned char *additional = luaL_checklstring(L, 2, &len);
	lua_pushinteger(L, mbedtls_ctr_drbg_reseed(ctx, additional, len));
	return 1;
}

static l_mbedtls_ctr_drbg_update(lua_State *L) {

	mbedtls_ctr_drbg_context *ctx = (mbedtls_ctr_drbg_context *) luaL_checkudata(L, 1, CLASS_NAME);
	size_t add_len;
	const unsigned char *additional = luaL_checklstring(L, 2, &add_len);
	mbedtls_ctr_drbg_update(ctx, additional, add_len);
	return 0;
}

static l_mbedtls_ctr_drbg_random_with_add(lua_State *L) {

	mbedtls_ctr_drbg_context *ctx = (mbedtls_ctr_drbg_context *) luaL_checkudata(L, 1, CLASS_NAME);
	size_t output_len;
	unsigned char *output = luaL_checklbuffer(L, 2, &output_len);
	size_t add_len;
	const unsigned char *additional = luaL_checklstring(L, 3, &add_len);
	lua_pushinteger(L, mbedtls_ctr_drbg_random_with_add(ctx, output, output_len, additional, add_len));
	return 1;
}

static l_mbedtls_ctr_drbg_random(lua_State *L) {
