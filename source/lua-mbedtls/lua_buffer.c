...
static int l_from_string(lua_State *L) {
	...
	if(ss > buf->sz) {
		char msg[1024];
		sprintf(msg, "str sz(%d) must less than buf sz(%d)", ss, buf->sz);
...
static l_tostring(lua_State *L) {

	BUF *buf = (BUF *) luaL_checkudata(L, 1, CLASS_NAME);
	if(buf->sig != SIGNATURE)
		luaL_argerror(L, 1, "Invalid buffer");
	lua_pushfstring(L, "%s '%s'(%d bytes): %p", CLASS_NAME, buf->name, buf->sz, buf);
	return 1;
}

static l_gc(lua_State *L) {
